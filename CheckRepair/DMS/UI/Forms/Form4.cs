...
{
    public partial class Form4 : TX.Framework.WindowUI.Forms.BaseForm
    {
        ...
        public Form4(int deviceReceiveID, int UserTaskDetailID, string status)
        {
            InitializeComponent();
            Status = status;
            ...
            labelName1.Text = ftd.LabelNames.Split(',')[0];
            labelUnit1.Text = ftd.Units.Split(',')[0];
            labelName2.Text = ftd.LabelNames.Split(',')[1];
            labelUnit2.Text = ftd.Units.Split(',')[1];
        ...
        {
            if (tbA.Text == "" || tbA.Text == null || tbB.Text == "" || tbB.Text == null)
            {
                this.Info("提示", "未输入检测值");
                return;
            }
            else
            {
                ...
                crc3.CheckKeys = "a" + utd.TemplateMainId + "_" + utd.TemplateDetailId + "_" + 3 + "b";//以'b'结尾表示checkBox
